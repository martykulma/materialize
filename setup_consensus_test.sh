...
pg_image=pg-cron:latest
sources=8
tables_per_source=600
...
gen_mz_sql() {
    ...
    for i in $(seq 1 $sources) ; do
        echo "CREATE CLUSTER sut$i (SIZE = 'scale=1,workers=2');"
        #echo "BEGIN;"
        echo "CREATE SCHEMA schema_$i;";
        echo "CREATE SOURCE schema_$i.pg_src_$i IN CLUSTER sut$i FROM POSTGRES CONNECTION pgcon (PUBLICATION '$pg_src_pub') FOR ALL TABLES;"
...
gen_pg_sql() {
    ...
    for i in $(seq 1 $tables_per_source) ; do
        echo "CREATE TABLE foo_$i (id int, data text, ts timestamp without time zone);"
        echo "INSERT INTO foo_$i SELECT i, sha512(i::text::bytea)::text, now() FROM generate_series(1,1000) AS i;"
    ...
    # this will log execution once a second, but limit the history to just the last minute
    echo "SELECT cron.schedule('heartbeat', '1 second', \$\$DELETE FROM cron.job_run_details WHERE end_time < now() - INTERVAL '1 minute';\$\$);"
}
...
ALTER SYSTEM SET pg_direct_max_concurrent_appends=8;
-- ALTER SYSTEM SET persist_next_listen_batch_retryer_fixed_sleep=0;
-- ALTER SYSTEM SET persist_next_listen_batch_retryer_initial_backoff='10ms';
-- ALTER SYSTEM SET persist_next_listen_batch_retryer_clamp='3s';
-- ALTER SYSTEM SET direct_pg_source=false;
_ALTER_SYS
...
#envd_params="--release --tokio-console --reset -- --all-features --unsafe-mode"
